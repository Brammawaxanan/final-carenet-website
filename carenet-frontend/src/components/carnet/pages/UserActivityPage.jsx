...
export default function UserActivityPage() {
  ...
  const handleTaskCreate = () => { setSelectedTask(null); setShowTaskModal(true); };
  const handleTaskEdit = (task) => { setSelectedTask(task); setShowTaskModal(true); };
  ...
  const handleGenerateReport = async () => {
    try {
      ...
      console.log('📊 Generating user report for userId:', userId);
      
      if (!userId) {
        throw new Error('User ID not found in localStorage');
      }
      
      const blob = await ApiService.generateUserReport(userId);
      ...
      link.href = url;
      link.download = `user-activity-report-${userId}.pdf`;
    ...
    <div className="space-y-8">
        ...
        {activityData && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center">
              ...
              <div className="text-2xl font-bold">${((activityData.runningDueCents || 0) / 100).toFixed(2)}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {isPremium && (
                <button onClick={handleTaskCreate} className="btn-primary flex items-center  space-x-2"> {/* flex items-center space-x-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg transition-colors*/}
        ...
        <div className="space-y-6">
          ...
          {activityData?.ledger && (
            <PaymentHistory ledger={activityData.ledger} runningDue={activityData.runningDueCents} />
          )}
        </div>
      </div>
