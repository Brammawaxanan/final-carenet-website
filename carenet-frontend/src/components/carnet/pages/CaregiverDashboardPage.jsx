...
import QRCodeModal from '../components/carnet/components/QRCodeModal'
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center shadow-lg border-2 border-white/30">
              <span className="text-white text-2xl font-bold">
                {dashboardData.caregiver.name?.[0]?.toUpperCase() || "C"}
              </span>
            </div>
            ...
            <div>
              <div className="flex items-center space-x-3 mb-2">
                <h1 className="text-3xl font-bold">
                  Welcome back, {dashboardData.caregiver.name.split(' ')[0]}!
                </h1>
                {/* Premium Badge */}
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>Rs {(dashboardData.caregiver.hourlyRate || 0).toLocaleString('en-LK')}/hour</div>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                {dashboardData.caregiverApplication && (dashboardData.caregiverApplication.verified || dashboardData.caregiverApplication.verificationCode) && (
                  <button
                    onClick={() => { setCaregiverData({
                      id: dashboardData.caregiverApplication.applicationId,
                      name: dashboardData.caregiver.name,
                      email: dashboardData.user?.email || '',
                      careType: dashboardData.caregiver.serviceTypes || '',
                      registrationDate: new Date().toISOString(),
                      verificationCode: dashboardData.caregiverApplication.verificationCode
                    }); setShowQRModal(true) }}
                    className="px-4 py-3 bg-white text-primary-600 rounded-lg hover:bg-pale-50 font-medium transition-colors"
                  >
                    View QR
                  </button>
                )}
                {/* Diagnostic: show verification code if present (helps confirm backend state) */}
                {dashboardData.caregiverApplication && dashboardData.caregiverApplication.verificationCode && (
                  <div className="ml-3 text-sm text-white/90">Code: {dashboardData.caregiverApplication.verificationCode}</div>
                )}
