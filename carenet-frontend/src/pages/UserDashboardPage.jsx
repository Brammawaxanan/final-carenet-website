...
const UserDashboardPage = () => {
  ...
  const [showAllBookingsForReview, setShowAllBookingsForReview] = useState(false)
  ...
  const fetchReviews = async () => {
    try {
      ...
      const resp = await ApiService.getReviews(userId)
      // expect resp.reviews or array
  ...
  const handleReviewUpdated = async (updated) => {
    // refresh lists after update
    try {
      await fetchReviews()
      await fetchDashboardData()
    } catch (err) {
      console.error('Failed to refresh after review update', err)
    }
  }
  ...
  const handleReviewDeleted = async (reviewId) => {
    try {
      ...
      console.error('Failed to refresh after review delete', err)
    }
  }

  const handleSubmitReview = async (assignmentId, rating, comment, onSuccess, onError) => {
    try {
      const payload = { assignmentId, rating, comment }
      const resp = await ApiService.submitReview(payload)
      // refresh reviews and dashboard
      fetchReviews()
      fetchDashboardData()
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {dashboardData.upcomingPayments.map(payment => (
            <div key={payment.id} className="p-4 border border-pale-200 rounded-lg">
              ...
              <div className="flex items-center justify-between">
                <span className="text-2xl font-bold text-pale-900">Rs. {payment.amount.toFixed(2)}</span>
                <Link
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-pale-900">Reviews & Ratings</h2>
          <span className="text-sm text-pale-600">Share feedback on completed bookings</span>
        </div>
              ...
              <div>
                <div className="space-y-3">
                  {reviews.length === 0 ? (
                    ...
                    reviews.map(r => (
                      <ReviewItem key={r.id} review={r} currentUserId={ApiService.getUserId()} onUpdated={handleReviewUpdated} onDeleted={handleReviewDeleted} />
                    ))
                  )}
                </div>
                <div className="mt-4 flex items-center justify-center py-4">
                  <button onClick={() => setShowAllBookingsForReview(true)} className="px-6 py-3 bg-primary-600 text-white rounded-lg">Write a review</button>
                </div>
              </div>
                ) : (
              <div className="space-y-4">
                {dashboardData.recentAssignments.filter(a => a.reviewable || a.completedAt).map(assignment => (
                  <div key={assignment.id} className="p-4 border border-pale-200 rounded-lg">
                    <div className="flex items-start justify-between mb-3">
                      <div className="cursor-pointer" onClick={() => setExpandedAssignmentId(expandedAssignmentId === assignment.id ? null : assignment.id)}>
                        <h4 className="font-semibold text-pale-900">{assignment.caregiverName}</h4>
                        <p className="text-sm text-pale-600">{assignment.service} • {assignment.completedAt ? `Completed ${formatDate(assignment.completedAt)}` : 'All tasks verified'}</p>
                      </div>
                      <div className="text-sm text-pale-500">{assignment.rating ? `Reviewed (${assignment.rating}/5)` : 'Not reviewed'}</div>
                    </div>
                    ...
                    {expandedAssignmentId === assignment.id ? (
                      !assignment.rating ? (
                        <ReviewForm
                          ...
                          onSubmit={async (rating, comment, done, fail) => {
                            await handleSubmitReview(assignment.id, rating, comment, () => {
                        ...
                        <div className="text-sm text-pale-600">Your review: <strong>{assignment.rating}/5</strong><div className="mt-1">{assignment.review || ''}</div></div>
                      )
                    ) : (
                      <div className="mt-2 text-sm text-pale-500">Click to {assignment.rating ? 'view' : 'add'} review</div>
                    )}
                  </div>
                ))}
                <div className="mt-4 flex justify-end">
                  <button onClick={() => setShowAllBookingsForReview(false)} className="px-4 py-2 bg-pale-100 rounded">Close</button>
                </div>
              </div>
            )}
          </div>
